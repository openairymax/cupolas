 ...
 *
 * @attention 实验性备用实现（未纳入 cupolas 构建，不编译/不安装）：
 *   本模块基于外部 docker CLI（exec/run/pull/stop/rm/pause/...），无任何
 *   生产接线，且执行后端与 workbench.c 的进程模式（fork/execvp +
 *   workbench_limits）构成双权威。按 Unify Design SSoT 单一权威源原则，
 *   执行后端的唯一权威为 workbench.c 进程模式。源文件保留
