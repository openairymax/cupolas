...
static void print_result(const benchmark_result_t *result)
{
    printf("  %-40s %8.2f us (avg) | min=%8.2f | max=%8.2f | p50=%8.2f | p99=%8.2f\n", result->name,
           result->avg_us, result->min_us, result->max_us, result->p50_us, result->p99_us);
}
...
void benchmark_audit_queue(int iterations)
{
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        audit_entry_t *entry =
            audit_entry_create(AUDIT_EVENT_PERMISSION, buf, "action", "resource", "detail", 1);
...
void benchmark_circuit_breaker(int iterations)
{
    ...
    circuit_breaker_config_t config = {.failure_threshold = 5,
                                       .success_threshold = 3,
                                       .timeout_ms = 60000,
    ...
    if (!breaker) {
        printf("  FAILED to create breaker\n");
        return;
    }
