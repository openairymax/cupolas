...
void test_error_codes(void)
{
    ...
    TEST_ASSERT(err == cupolas_ERR_PERMISSION_DENIED, "from_vault_DENIED",
                "Should convert properly");
...
void test_signature_module(void)
{
    ...
    TEST_ASSERT(algo_str != NULL && strlen(algo_str) > 0, "sig_algo_RSA256",
                "Should return algo string");
...
void test_vault_module(void)
{
    ...
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "password") == 0, "vault_type_password",
                "Should return password string");

    type_str = cupolas_vault_cred_type_string(CUPOLAS_VAULT_CRED_TOKEN);
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "token") == 0, "vault_type_token",
                "Should return token string");

    type_str = cupolas_vault_cred_type_string(CUPOLAS_VAULT_CRED_KEY);
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "key") == 0, "vault_type_key",
                "Should return key string");

    type_str = cupolas_vault_cred_type_string(CUPOLAS_VAULT_CRED_CERTIFICATE);
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "certificate") == 0, "vault_type_certificate",
                "Should return certificate string");

    type_str = cupolas_vault_cred_type_string(CUPOLAS_VAULT_CRED_SECRET);
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "secret") == 0, "vault_type_secret",
                "Should return secret string");

    type_str = cupolas_vault_cred_type_string(CUPOLAS_VAULT_CRED_NOTE);
    TEST_ASSERT(type_str != NULL && strcmp(type_str, "note") == 0, "vault_type_note",
                "Should return note string");

    /* Test operation strings */
    const char *op_str;

    op_str = cupolas_vault_operation_string(CUPOLAS_VAULT_OP_READ);
    TEST_ASSERT(op_str != NULL && strcmp(op_str, "read") == 0, "vault_op_read",
                "Should return read string");

    op_str = cupolas_vault_operation_string(CUPOLAS_VAULT_OP_WRITE);
    TEST_ASSERT(op_str != NULL && strcmp(op_str, "write") == 0, "vault_op_write",
                "Should return write string");

    op_str = cupolas_vault_operation_string(CUPOLAS_VAULT_OP_DELETE);
    TEST_ASSERT(op_str != NULL && strcmp(op_str, "delete") == 0, "vault_op_delete",
                "Should return delete string");
    ...
    TEST_ASSERT(pub_len > 0 && pub_len < sizeof(public_key), "pub_key_length",
                "Public key length should be valid");
    TEST_ASSERT(priv_len > 0 && priv_len < sizeof(private_key), "priv_key_length",
                "Private key length should be valid");
...
void test_runtime_protection_module(void)
{
    ...
    TEST_ASSERT(viol_str != NULL, "viol_type_INTEGRITY",
                "Should return integrity violation string");
    ...
    TEST_ASSERT(supported == true || supported == false, "feature_seccomp",
...
void test_network_security_module(void)
{
    ...
    TEST_ASSERT(tls_str != NULL && strstr(tls_str, "1.2") != NULL, "tls_version_1_2",
                "Should contain 1.2");

    tls_str = cupolas_tls_version_string(CUPOLAS_TLS_1_3);
    TEST_ASSERT(tls_str != NULL && strstr(tls_str, "1.3") != NULL, "tls_version_1_3",
                "Should contain 1.3");
